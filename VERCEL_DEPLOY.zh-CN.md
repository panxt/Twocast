...
| `SUPABASE_SECRET_KEY` | 项目的 server-side secret key，从 Supabase 项目 API Keys 页面取得；不要使用 publishable/anon key |
...
`DATABASE_URL`、`SUPABASE_SECRET_KEY`、管理员码及加密密钥不得提交到 Git。当前本机 `.env.vercel.local` 已有大部分值；`SUPABASE_SECRET_KEY` 与部署 URL 仍需补齐。该文件还包含只供本机维护用的 `SUPABASE_DB_ROLE_PASSWORD`，导入 Vercel 变量时不要把这一项上传。
...
2. 配置上述 Vercel 环境变量；在 Supabase 项目 Settings → API Keys 复制 secret key 至 Vercel。保存后重新部署。
