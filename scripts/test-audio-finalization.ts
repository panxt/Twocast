...
import { finalizeMp3, normalizeExistingMp3 } from '../src/lib/podcast/finalize_mp3'
...
async function main() {
  ...
  try {
    ...
    assert.ok(output.audio.includes(Buffer.from('测试节目')))
    ...
    assert.ok(legacy.audio.includes(Buffer.from('旧音频')))
